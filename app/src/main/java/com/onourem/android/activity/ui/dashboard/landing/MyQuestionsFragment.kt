...
class MyQuestionsFragment(val rvScrollListener: RVScrollListener) :
    ...
    private var selectedFilterIndex = 2
    ...
    companion object {
        ...
        fun create(rvScrollListener: RVScrollListener): MyQuestionsFragment {
            return MyQuestionsFragment(rvScrollListener)
        }
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        dashboardData = (fragmentContext as DashboardActivity).getDashboardDataMQ()
        ...
        binding.swipeRefreshLayout.setOnRefreshListener {
            ...
            if (dashboardData != null) {
                (fragmentContext as DashboardActivity).setDashboardDataMQ(null)
            }
            refreshUI()
        }
        ...
        binding.rvPlayGames.addOnScrollListener(object : PaginationListener(layoutManager!!) {
            override fun loadMoreItems() {
                MyQuestionsFragment.isLoading = true
                loadMoreGames()
            }

            override fun isLastPage(): Boolean {
                return MyQuestionsFragment.isLastPage
            }

            override fun isLoading(): Boolean {
                return MyQuestionsFragment.isLoading
            }
        })
...
//        })
        ...
        questionGamesViewModel!!.updateItem().observe(viewLifecycleOwner)
        { item: LoginDayActivityInfoList? ->
            if (item != null && soloGamesAdapter != null) {
        ...
        if (dashboardData != null) {
            ...
            loginDayActivityInfoList =
                (dashboardData!!.loginDayActivityInfoList as MutableList<LoginDayActivityInfoList>?)
...
//            filterItems = dashboardData.filterItems
            ...
            displayNumberOfActivity = dashboardData!!.displayNumberOfActivity!!
    ...
    private fun loadData(showProgress: Boolean, refreshing: Boolean) {
        ...
        preferenceHelper!!.putValue(Constants.KEY_SELECTED_FILTER_INDEX, 3)
        //AppUtilities.showLog("PlayGamesFragment", "getLoginUserCreatedActivityInfo");
        if (loginDayActivityInfoList == null || loginDayActivityInfoList!!.isEmpty() && !isDataLoading) {
            isDataLoading = true
            questionGamesViewModel!!.loginUserCreatedActivityInfo().observe(
                viewLifecycleOwner
                ...
                handleResponse(
                    ...
                    true,
                    showProgress,
                    "getLoginUserCreatedActivityInfo"
                )
            }
        } else {
            setAdapter()
...
//                        0
//                    )
//                    (fragmentContext as DashboardActivity).setDashboardDataMQ(dashboardData)
                    ...
                    footerMessage = if (playGroup.playGroupId.equals("FFF", ignoreCase = true)) {
                        ...
                        "You have not created any questions. Use the 'Create Q' option from the main menu to create your own questions ans ask friends."
                    } else {
...
//            }
        ...
        if ((fragmentContext as DashboardActivity).getDashboardRecyclerViewPositionMQ() != -1) layoutManager!!.scrollToPositionWithOffset(
            (fragmentContext as DashboardActivity).getDashboardRecyclerViewPositionMQ(),
            (fragmentContext as DashboardActivity).getDashboardRecyclerViewPositionTopViewMQ()
        )
    ...
    private fun loadMoreGames() {
        ...
        val gameIdList3 = getDisplayGameIdListElements(
            gameResIdList!!, start
        )
        AppUtilities.showLog("**gameIdList2:", gameIdList3.size.toString())
        var i = 0
        while (i < gameIdList3.size) {
            val item = gameIdList3[i]
            activityIds.add(item.toInt())
            i++
        }
        questionGamesViewModel!!.getYYYNextUserLoginDayActivityInfo(
            ...
            if (apiResponse.loading) {
                ...
                isLoading = false
                if (apiResponse.body.errorCode.equals("000", ignoreCase = true)) {
                    //AppUtilities.showLog("PlayGamesFragment", "LoginDayActivityInfoList getYYYNextUserLoginDayActivityInfo" + apiResponse.body.getLoginDayActivityInfoList().size() + "");
...
//        counter++;
        ...
        (fragmentContext as DashboardActivity).setDashboardRecyclerViewPositionMQ(
            positionIndex,
            topView
        )
        (fragmentContext as DashboardActivity).setDashboardSelectedFilter(
            selectedFilterIndex,
        )
        isDataLoading = false
    }

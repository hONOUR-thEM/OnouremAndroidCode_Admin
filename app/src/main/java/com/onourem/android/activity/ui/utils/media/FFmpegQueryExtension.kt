...
class FFmpegQueryExtension {
    ...
    fun imageToVideo(
        ...
        val fade =
            "fps=$FRAME_RATE,fade=type=in:duration=1,fade=type=out:duration=0.5:start_time=$fadeEndDuration"
        val inputs: ArrayList<String> = ArrayList()
        inputs.apply {
            add("-loop")
            add("1")
...
//                }
    ...
    fun mergeAudiosWithDifferentVolume(
        inputAudioList: ArrayList<Paths>,
        duration: String,
        output: String,
    ): Array<String> {
...
//                }
            ...
            add("-filter_complex")
//            add("amovie=${inputAudioList[1]}:loop=999[s];[0][s]")
            add("amix=inputs=${inputAudioList.size}:duration=$duration:dropout_transition=${inputAudioList.size}")
            add("-codec:a")
            add("libmp3lame")
            add("-q:a")
    ...
    fun cutAudio(
        ...
        AppUtilities.showLog("CUT", inputs.toString())
        return inputs.toArray(arrayOfNulls<String>(inputs.size))
    }

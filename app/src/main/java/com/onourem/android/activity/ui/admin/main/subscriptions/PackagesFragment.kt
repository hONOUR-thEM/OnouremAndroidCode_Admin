...
class PackagesFragment : AbstractBaseViewModelBindingFragment<AdminViewModel, FragmentSettingsBinding>() {
    ...
    private fun loadData() {
        ...
        viewModel.getPackageAndInstitutionCode().observe(viewLifecycleOwner) { apiResponse: ApiResponse<GetPackageAndInstitutionCodeResponse> ->
            if (apiResponse.loading) {
                ...
                setAdapter(apiResponse.body.packageIdAndNameList as ArrayList<PackageNameId>)
            } else {
                hideProgress()
                showAlert(
    ...
    private fun setAdapter(settingsItemList: ArrayList<PackageNameId>) {
        binding.rvSettings.adapter = PackageAdapter(settingsItemList) {

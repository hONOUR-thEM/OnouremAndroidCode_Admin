...
class ScheduleFutureQuestionsFragment :
    ...
    companion object {
        fun create(): ScheduleFutureQuestionsFragment {
            val fragment = ScheduleFutureQuestionsFragment()
            val bundle = Bundle()
    ...
    private fun setAdapter(getAdminActivityListResponse: GetAdminActivityListResponse, selectedRadioButton: String) {
        ...
        val list: ArrayList<AdminActivityResponse> = when (selectedRadioButton) {
            "Unscheduled" -> {
                getAdminActivityListResponse.adminActivityResponseList.filter { !it.activityScheduled && it.activityType == "1toM"} as ArrayList<AdminActivityResponse>
            }
            "Scheduled" -> {
                getAdminActivityListResponse.adminActivityResponseList.filter { it.activityScheduled && !it.activityTriggered && it.activityType == "1toM"} as ArrayList<AdminActivityResponse>
            }
            else -> {
                getAdminActivityListResponse.adminActivityResponseList.filter { it.activityTriggered && it.activityType == "1toM"} as ArrayList<AdminActivityResponse>
            }
        }
                ...
                when {
                    ...
                    it.first == FutureTaskListAdapter.CLICK_UN_SCHEDULE -> {
                        ...
                        val dpd = DatePickerDialog.newInstance(
                            this@ScheduleFutureQuestionsFragment,
        ...
        binding.rvQuestionsGames.addOnScrollListener(object : PaginationListener(layoutManager) {
            override fun loadMoreItems() {
                this@ScheduleFutureQuestionsFragment.isLoading = true
                loadMoreGames()
            }

            override fun isLastPage(): Boolean {
                return this@ScheduleFutureQuestionsFragment.isLastPage
            }

            override fun isLoading(): Boolean {
                return this@ScheduleFutureQuestionsFragment.isLoading
            }
        })
                    ...
                    if (apiResponse.body.adminActivityResponseList.isEmpty()) {
                        ...
                        if (futureQuestionListAdapter != null) {
                            ...
                            val list: ArrayList<AdminActivityResponse> = if (selectedRadioButton == "Unscheduled"){
                                apiResponse.body.adminActivityResponseList.filter { !it.activityTriggered && it.activityType == "1toM" } as ArrayList<AdminActivityResponse>
                            }else{
                                apiResponse.body.adminActivityResponseList.filter { it.activityTriggered && it.activityType == "1toM"} as ArrayList<AdminActivityResponse>
                            }

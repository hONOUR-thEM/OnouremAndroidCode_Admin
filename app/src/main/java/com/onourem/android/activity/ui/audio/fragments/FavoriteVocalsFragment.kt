...
import com.onourem.android.activity.ui.audio.adapters.FavoriteVocalsAdapter
...
class FavoriteVocalsFragment :
    ...
    private var selectedFilterIndex = 3
    ...
    private var songAdapter: FavoriteVocalsAdapter? = null
    ...
    companion object {
        fun create(): FavoriteVocalsFragment {
            return FavoriteVocalsFragment(
            )
        }
    }
...
//                            it))
        ...
        userActionViewModel!!.actionMutableLiveDataFavorite.observe(viewLifecycleOwner) { action: Action<Any?>? ->
            if (action != null && action.actionType != ActionType.DISMISS) {
                userActionViewModel!!.actionConsumedFavorite()
...
//                            Manifest.permission.READ_CONTACTS,
                                ...
                                coroutineScope.launch {
                                    withContext(Dispatchers.Main) {
                                        handlePermissionResultForUpSideDownCake(
                                            item,
                                            PermissionManager.requestPermissions(
                                                this@FavoriteVocalsFragment, MY_PERMISSIONS_REQUEST_MEDIA_PLAYBACK,
    ...
    private fun loadData() {
        ...
        getAudioShorts(Constants.FAVORITE_VOCALS)
...
//        counter++;
    ...
    private fun setAdapter() {
        ...
        if (songAdapter == null) {
            songAdapter =
                FavoriteVocalsAdapter(songsFromServer, preferenceHelper) { item ->
...
//                                )
        ...
        binding.rvAudioGames.addOnScrollListener(object : PaginationListener(layoutManager!!) {
            ...
            override fun isLastPage(): Boolean {
                return this@FavoriteVocalsFragment.isLastPagePagination
            }

            override fun isLoading(): Boolean {
                return this@FavoriteVocalsFragment.isLoadingPage
            }
        })
            ...
            if (apiResponse.loading) {
                ...
                if (apiResponse.body.errorCode.equals("000", ignoreCase = true)) {
                    ...
                    if (selectPrivacyViewModel!!.selectedPrivacyMutableLiveData!!
                        ...
                        this@FavoriteVocalsFragment.privacyGroupsDefault.clear()
                        this@FavoriteVocalsFragment.privacyGroupsDefault.addAll(
                            oldSelectedAfterResponse
                        )
                        this@FavoriteVocalsFragment.privacyGroups.clear()
                        if (privacyGroups != null) {
                            this@FavoriteVocalsFragment.privacyGroups.addAll(privacyGroups)
                        }
                        ...
                        this@FavoriteVocalsFragment.privacyGroupsDefault.clear()
                        this@FavoriteVocalsFragment.privacyGroupsDefault.addAll(defaultSelected)
                        this@FavoriteVocalsFragment.privacyGroups.clear()
                        if (privacyGroups != null) {
                            this@FavoriteVocalsFragment.privacyGroups.addAll(privacyGroups)
                        }
                    ...
                    navController.navigate(
                        MobileNavigationDirections.actionGlobalNavAudioPrivacy(
                            this@FavoriteVocalsFragment.privacyGroups.toTypedArray(),
                            this@FavoriteVocalsFragment.privacyGroupsDefault.toTypedArray(), song
                        )
                    )
                } else {
                    showAlert(apiResponse.body.errorMessage)
